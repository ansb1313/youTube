...
function* saga(){
    yield all([
        takeLatest(Action.Types.GET_SEARCH_DATA,function* ({data, reset}){
            ...
            let channelData = yield Promise.all(channelArr.map(async (item) => {
            ...
            let itemsChannelWithVideoData = yield Promise.all(videoArr?.map(async (item) => {
                const channelId = item.snippet.channelId;
                ...
                yield put(Action.Creators.updateState({
                    searchResults:{
                        ...
                        items:{
                            ...
                            ...result?.items,
                            newVideoData:[
                                ...prevList?.items?.newVideoData,
                                ...result?.items?.newVideoData,
                            ],
                            relatedVideoData:[
                                ...prevList?.items?.relatedVideoData,
