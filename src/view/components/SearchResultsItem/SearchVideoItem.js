        ...
        <Container onClick={onClick}>
            {
                data?.snippet &&
                <VideoItem
                    onMouseOver={() => { setThumbnailMenu(true) }}
                    onMouseLeave={() => { setThumbnailMenu(false)}}
                >
                    ...
                    <div className="videoDetails">
                        <h1 className={'videoTitle'}>{data?.snippet.title}</h1>
                        <span className={'viewCount'}>조회수 {viewCountCommaMark(data?.videoData?.items[0]?.statistics?.viewCount)}회 <BsDot/>{dateFromNow(data?.snippet?.publishedAt)}</span>
            ...
            <InfoMoreMenu className={cn({isActive:thumbnailMenu})} onClick={(e)=>{e.stopPropagation()}}>
                <VerticalThreeDot/>
            </InfoMoreMenu>
        </Container>
    )
